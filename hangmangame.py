...
class HangmanGame(HangmanGameData):  # kalıtım kullandım
    ...
    def soru1(self):
        print("SORU:",
              self.anahtar)  # sözlükteki kelimenin anahtarı soru olduğu için o anahtarı ekrana bastıran fonksiyon
    ...
    def question(self):
        ...
        for j in self.kelime:
            if j == harf:
                ...
                if (j != " "):
                    self.soru.append(
                        "-")  # BOŞLUKLARI DOLDURMAMAK İÇİN TEKRAR İF BLOĞU KOYDUM bunu yapmadığım zaman boşluklara da "-" işareti koyuyor. Bu istemediğim bir şey.

                else:
                    pass

        self.show = ' '.join([str(i) for i in
                              self.soru])  # self.soru listesi içindeki ifadeleri yatay şekilde bastırmak için kullandığım kod satırı
            ...
            for i in b:
                ...
                if harf == i:
                    ...
                    for k in range(len(a)):  # indexleri üzerinde geziyorum
                        if a[
                            k] == harf:  # eğer indexteki harf tahmin edilen harf ile aynıysa, o harfin indexini listeye atıyorum
                            index_list.append(k)
                    ...
                    self.show = ' '.join(
                        [str(k) for k in self.soru])  # yatay şekilde stringi çevirmek için kullandığım kod

            print(self.show)  # tahminden sonraki kelimeyi tekrar bastırıyorum
            print(
                "Tahmininiz yok ise 'YOK' yazınız")  # kullanıcıya verdiği harf tahminlerinden sonra soruyu tamamen cevaplamak isterse diye seçenek sunuyorum
